...
## 뮤텍스(Mutex)
뮤텍스란 임계구역을 가진 스레드들의 실행 시간이 서로 겹치지 않고 각각 단독으로 실행(상호배제) 되도록 하는 기술을 말한다.
<br>
태스크가 자원에 대한 독점적인 액세스를 얻고자할 때 사용하며 일종의 바이너리 세마포어이다.
<br>
우선순위전도(Priority Inversion) 문제를 해결하는 능력이 있다.
<br>
<br>
뮤텍스는 세마포어라는 단어의 뜻이 들어있으며 세마포어의 기술로 만든다.

## IPC(Inter Process Communication) 통신
ICP 통신이란 프로세스 사이에 서로 데이터를 주고받는 행위 또는 그에 대한 방법이나 경로를 뜻한다.
<br>
태스크나 ISR이 다른 태스크에게 정보를 전달할 때 필요하다.
<br>
<br>
IPC 통신 방법에는 여러 가지가 존재한다.
<ul>
<li>전역 변수 이용(각 태스크나 ISR 변수에 독점적 액세스 보장)</li>
<li>메시지 전송 : 메시지 메일박스(MAILBOX), 메시지 큐(Message Queue)</li>
<li>이벤트 플래그 : 간단한 이벤트 전송</li>
<li>세마포어 : 간단한 이벤트 전송</li>
</ul>
여기서 사용하는 세마포어는 임계구역에서 사용하는 세마포어는 다른 세마포어로 구분 필요
<br>
